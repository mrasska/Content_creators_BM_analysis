...
#List of channels info to retrieve from API request
name_cp=[]
id_cp=[]
name_video=[]
id_video=[]
...
#1_YT_cat.csv contains all YT video category codes for requests 
dfg=pd.read_csv('1_YT_cat.csv')
list_id=dfg['cat_id'].to_list()

#API requesting for trending videos in each video genre
...
#Saving data from the API requests
...
#Saving contact details (email address) of each channel
def save_contact_csv(name_cp, id_cp, description):
    ...
    #to extract the mail adress / For extract all method - there is a need to change the RE --> pat='([\w.+-]+@[\w-]+\.[\w.-]+)'
...
# Requesting additional info about selected content creators (country)
...
def save_info(df, country):
    #Saving email addresses for all content creators 
    df['country']=country
    df.to_csv('mail.csv', index=True, header=True)
    
    #Separate file for email addresses of content creators located in France
